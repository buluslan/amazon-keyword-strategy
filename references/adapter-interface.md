...
### 4.2 SIF MCP（语义字段，未实测 ⏳）

字段名为调研阶段语义推断，实际 MCP 返回 key 待实测确认。能跑通后补真实 key。
...
> ⏳ = 未实测。MVP 不依赖 SIF MCP；实测核验后把 ⏳ 换成确认字段名。
...
| 待核 ⏳ | 2 列未列举 | 实测样本 `数据接入/SIF/实测样本/` 核验后补 |
...
非 MVP 范围。实现时在工厂之上加一层 `MultiSourceFetcher`，合并多源 `fetch` 结果按字段优先级取值。
